...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -a, --all          Stage all changes (including untracked files) [default]"
    echo "  -m, --modified     Stage only modified files (not untracked)"
    echo "  -p, --push         Push to remote after committing"
    echo "  -d, --dry-run      Show what would be committed without actually committing"
    echo "  -h, --help         Show this help message"
    echo ""
    echo "Examples:"
    echo "  commit-helper                  # Stage all changes and commit [default]"
    echo "  commit-helper --modified       # Stage only modified files"
    echo "  commit-helper --push           # Stage all, commit, and push"
}
...
if [ -z "$STAGED_DIFF" ]; then
    ...
    echo ""
    echo "💡 Tip: Untracked files are included by default, use --modified to exclude them"
    exit 1
fi

# Show what will be committed (only if not using -a flag)
if [ "$STAGE_ALL" != true ]; then
...
COMMIT_MSG=$(claude -p "Based on these git changes, write a concise commit message following conventional commit format (feat:, fix:, docs:, style:, refactor:, test:, chore:).
